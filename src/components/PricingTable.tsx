        ...
        <section className="bg-slate-200 py-20 px-4 rounded flex flex-col items-center">
            <header className="bg-white p-1 rounded-full mb-12 flex">
                <button className={`px-6 py-2 rounded-full transition-all duration-300 cursor-pointer ${ 
                    !isAnnually ? 'bg-zinc-900 text-white' : 'text-zinc-500 hover:text-zinc-900'}`}
            ...
            <div className="grid gap-8 md:grid-cols-3 max-w-5xl w-full ">
                {cards.map((card) => (
                    <div key={card.titleName} className={`cursor-pointer border-1 p-4 bg-slate-50 p-8 rounded-xl shadow-lg ${card.isRecommended ? 'border-2 border-indigo-500 shadow-indigo-500/50' : 'border-black shadow-zinc-500'}`}>
